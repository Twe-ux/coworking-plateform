...
import type { ClockInRequest, ApiResponse, TimeEntry as TimeEntryType } from '@/types/timeEntry'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (activeShifts.length > 0) {
      return NextResponse.json<ApiResponse<null>>({
        success: false,
        error: 'Vous avez déjà un shift actif. Veuillez pointer la sortie avant de commencer un nouveau shift.',
