...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validation basique
    const { spaceId, date, startTime, endTime, duration, durationType, guests } = body
    
    if (!spaceId || !date || !startTime || !endTime) {
      return NextResponse.json(
        { error: 'Données manquantes: spaceId, date, startTime, endTime sont requis' },
        { status: 400 }
      )
    }
    ...
    console.error('❌ Erreur lors de la création de la réservation test:', error)
