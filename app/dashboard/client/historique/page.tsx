...
export default function ClientHistoriquePage() {
  ...
  const stats = useMemo(() => {
    ...
    const reservations = activities.filter(
      (a) => a.type === 'reservation'
    ).length
  ...
  const getActivityColor = (type: ActivityType) => {
    switch (type) {
      case 'reservation':
        return 'var(--color-coffee-accent)'
      case 'order':
        return 'var(--color-coffee-primary)'
      case 'payment':
        return 'var(--color-coffee-warm)'
      case 'profile':
        return 'var(--color-client-muted)'
      case 'login':
        return 'var(--color-coffee-light)'
      default:
        return 'var(--color-client-muted)'
    }
  }
              ...
              {activityTypes.map((type) => {
                ...
                return (
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setFilters((prev) => ({ ...prev, type: type.id as any }))
                    }
                    style={
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {['all', 'success', 'pending', 'failed', 'cancelled'].map(
                        (status) => (
                          <Button
                            key={status}
                            variant={
                              filters.status === status ? 'default' : 'ghost'
                            }
                            size="sm"
                    ...
                    <div className="space-y-1">
                      {[
                        ...
                        <Button
                          key={sort.id}
                          variant={
                            filters.sortBy === sort.id ? 'default' : 'ghost'
                          }
                          size="sm"
                          className="w-full justify-start"
                          onClick={() =>
                            ...
                            >
                              ...
                              {activity.type === 'order' &&
                                activity.metadata.items && (
                                  <span className="mr-3 inline-block">
                                    {activity.metadata.items} article
                              ...
                              {activity.metadata.method && (
                                <span className="mr-3 inline-block">
                                  Paiement:{' '}
                                  {activity.metadata.method.replace(
                                    'stripe_',
                                    ''
                                  )}
                                </span>
