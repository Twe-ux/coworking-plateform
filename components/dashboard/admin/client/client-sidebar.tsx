...
import { sidebarVariants, navItemVariants, containerVariants } from '@/lib/animations'
    ...
    >
      ...
      <div className="border-b border-[var(--color-client-border)] p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="font-bold text-lg" style={{ color: 'var(--color-client-text)' }}>
              Cow or King
            </h1>
        ...
        {/* Profil utilisateur */}
        <div className="flex items-center gap-3 p-3 rounded-lg border border-[var(--color-client-border)]">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate" style={{ color: 'var(--color-client-text)' }}>
              {session.user.name || 'Utilisateur'}
            </p>
            <p className="text-xs truncate" style={{ color: 'var(--color-client-muted)' }}>
              Membre café
            </p>
          </div>
        </div>
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'shadow-sm border border-[var(--color-coffee-primary)]/20'
                      : 'hover:bg-[var(--color-client-card)]/80'
                  )}
                  style={{
                    backgroundColor: isActive ? 'var(--color-coffee-secondary)' : 'transparent',
                    color: isActive ? 'var(--color-coffee-primary)' : 'var(--color-client-text)'
                  }}
                >
                <Icon className={cn(
                  'h-5 w-5 transition-colors',
                  isActive ? 'text-[var(--color-coffee-primary)]' : 'text-[var(--color-client-muted)]'
                )} />
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs mt-0.5 truncate" style={{ color: 'var(--color-client-muted)' }}>
                    {item.description}
                  </p>
                </div>
              </Link>
      ...
      <div className="border-t border-[var(--color-client-border)] p-4">
        ...
        >
          <Clock className="h-5 w-5 mx-auto mb-2" style={{ color: 'var(--color-coffee-primary)' }} />
          <p className="text-xs font-medium" style={{ color: 'var(--color-client-text)' }}>
            Ouvert aujourd&apos;hui
          </p>
