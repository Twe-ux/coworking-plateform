...
describe('Mobile Sidebar Gestures', () => {
    ...
    it('requires swipe to start from edge area', () => {
      ...
      cy.get('body').then(($body) => {
        ...
        cy.wrap($body)
          ...
          .trigger('touchmove', {
            touches: [{ clientX: centerX + 100, clientY: centerY }],
          })
          .trigger('touchend')
      })
  ...
  describe('Sidebar Closing Gestures', () => {
    ...
    it('requires significant swipe distance to close', () => {
      ...
      cy.get('[data-testid="mobile-sidebar"]').then(($sidebar) => {
        ...
        cy.wrap($sidebar)
          .trigger('touchstart', {
            touches: [{ clientX: rect.right - 10, clientY: rect.height / 2 }],
          })
          .trigger('touchmove', {
            touches: [{ clientX: rect.right - 30, clientY: rect.height / 2 }],
          })
          .trigger('touchend')
      })
    ...
    it('closes on outside tap beyond overlay', () => {
      ...
      cy.get('body').then(($body) => {
        ...
        cy.wrap($body)
          .trigger('touchstart', {
            touches: [{ clientX: rect.width - 10, clientY: 100 }],
          })
          .trigger('touchend')
      })
  ...
  describe('Gesture Velocity and Physics', () => {
    ...
    it('handles momentum-based closing', () => {
      ...
      cy.get('[data-testid="mobile-sidebar"]').then(($sidebar) => {
        ...
        cy.wrap($sidebar)
          .trigger('touchstart', {
            touches: [{ clientX: rect.right - 20, clientY: rect.height / 2 }],
          })
          .wait(50)
          .trigger('touchmove', {
            touches: [{ clientX: rect.right - 80, clientY: rect.height / 2 }],
          })
          .trigger('touchend')
      })
  ...
  describe('Accessibility During Gestures', () => {
    it('announces sidebar state changes to screen readers', () => {
      ...
      cy.get('[data-testid="mobile-sidebar"]').should(
        'have.attr',
        'aria-expanded',
        'true'
      )
