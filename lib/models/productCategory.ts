...
const productCategorySchema = new Schema<IProductCategory>(
  {
    ...
    color: {
      ...
      validate: {
        ...
        message:
          'La couleur doit être un code hexadécimal valide (#RRGGBB ou #RGB)',
      },
    },
    image: {
      type: String,
...
productCategorySchema.virtual('hasChildren').get(function (
  this: IProductCategory
) {
  // Cette propriété sera calculée lors des requêtes avec aggregation
  return false
})

productCategorySchema.virtual('productCount').get(function (
  this: IProductCategory
) {
  // Cette propriété sera calculée lors des requêtes avec aggregation
  return 0
...
productCategorySchema.methods.getChildren = function (
  this: IProductCategory,
  activeOnly: boolean = true
) {
  ...
  return this.model('ProductCategory')
    .find(query)
    .sort({ sortOrder: 1, name: 1 })
}

productCategorySchema.methods.getDescendants = function (
  this: IProductCategory,
  maxDepth: number = 3
) {
  return this.model('ProductCategory').aggregate([
...
productCategorySchema.methods.updateSortOrder = function (
  this: IProductCategory,
  newOrder: number
) {
